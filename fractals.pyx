...
cdef koch_curve(Point p):
    ...
    return np.array(
        ...
        create_point(
            p.position + divide_by_scalar(t2, 2),
            divide_by_scalar(rotate(p.rotation, k), 3)
        ),
        create_point(
            p.position + divide_by_scalar(t3 / 2),
            divide_by_scalar(rotate(p.rotation, -k), 3)
        ),
        create_point(
            p.position + t4,
...
cdef sierpinski_triangle(Point p):
    ...
    v3 = rotate(v1, -k)
    return np.array([
        create_point(p.position + v1, v3),
        create_point(p.position + v2, v1),
        create_point(p.position + v3, v2),
    ])
...
cdef sierpinski_carpet(Point p):
    ...
    t1 = new_rot
    t2 = rotate(multiply_by_scalar(new_rot, sqrt(2)), pi / 4)

    result = np.zeros((8,), dtype=create_point)
    for i in range(4):
        result[0 + i * 2] = create_point(p.position + t1, new_rot)
        result[1 + i * 2] = create_point(p.position + t2, new_rot)
        t1 = rotate(t1, pi / 2)
        t2 = rotate(t2, pi / 2)
...
cdef dragon(Point p):
    cdef float k = sqrt(2) / 2
    cdef float q = - pi / 4
    t1 = rotate(p.rotation, q) * k
    t2 = rotate(p.rotation, 3 * q) * k
    return np.array([
        create_point(p.position - t1, t1),
        create_point(p.position + t2, t2),
    ])
...
cdef peano_curve(Point p):
    ...
    return np.array([
        create_point(p.position - t1 - t2, negative(t2)),
        create_point(p.position + t1 - t2, t1),
        create_point(p.position + t1 + t2, t1),
        create_point(p.position - t1 + t2, t2),
    ])
...
cdef minkowski_curve(Point *p):
    ...
    return np.array([
        create_point(
            p.position + multiply_by_scalar(t2, 3),
            divide_by_scalar(t1, 2)
        ),
        create_point(
            p.position + multiply_by_scalar(t2, 2) + t1,
            divide_by_scalar(t2, 2)
        ),
        create_point(
            p.position + t2 + multiply_by_scalar(t1, 2),
            divide_by_scalar(t1, 2)
        ),
        create_point(
            p.position + t1,
